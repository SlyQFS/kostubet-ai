...
teloxide 0.14 (macros, rustls), tokio, sqlx 0.8 (sqlite, migrate), reqwest 0.12 (rustls, stream), fastembed 5 (ONNX), rust-stemmers, serde, dotenvy, tracing, thiserror. Edition 2021, release-профиль: `lto`, `strip`, `panic = "abort"`.

## Структура

- `src/main.rs` — конфиг, пул, эмбеддер (spawn_blocking), запуск Dispatcher.
...
- `src/knowledge/` — `loader.rs` (карточки `knowledge/*.json`), `matcher.rs` (поиск по корням слов без окончаний через русский стемминг + точные ключи + fastembed cosine; при недоступном эмбеддере — деградация на keyword/стемминг).
...
- Юнит-тесты обязательны: парсеры, санитайзер, промпт-сборка, матчер (без эмбеддера), SQL-методы (in-memory).
...
Multi-stage: builder `rust:alpine` + cargo-chef, runtime `alpine` + ca-certificates, non-root. Тома: `/app/data` (БД, кэш моделей), `migrations/` и `knowledge/` копируются в образ. Рабочая директория `/app` — миграции и knowledge ищутся относительно неё.
